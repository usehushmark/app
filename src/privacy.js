...
let formRevision=0,lastWithdrawal=null;
...
async function unlock(){
 ...
 localStorage.setItem(fpKey,fingerprint);wasm=await sdk.WasmFactory.getInstance();if(epoch!==version)throw new Error('Session changed.');enc=new sdk.EncryptionService();enc.deriveEncryptionKeyFromSignature(first);first.fill(0);second.fill(0);owner=key;storage=scopedStorage(sessionStorage,key.toBase58());$('owner').textContent=key.toBase58();$('unlock-state').textContent='UNLOCKED';await publicRefresh();message('Unlocked. Sync private balances to scan existing notes, or deposit to the pool.');
}
...
async function execute(){
 ...
 operation=p;review=null;notes[p.index]=null;render();$('payment-review').hidden=true;message('Preparing encrypted notes and generating the payment proof…');
