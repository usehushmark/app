...
const asset=ASSETS.find(a=>a.symbol===(process.env.AUDIT_ASSET||'SOL'));assert.ok(asset,'Unsupported test asset');const index=ASSETS.indexOf(asset),spl=!!asset.mint,mint=asset.mint;
...
try{const page=await browser.newPage();const errors=[];page.on('pageerror',error=>errors.push(error.message));await page.addInitScript({content:`if(location.hostname==='127.0.0.1')sessionStorage.setItem('hushmark-rpc',${JSON.stringify(auditRpc)});window.__auditSeed=${JSON.stringify(auditSeed)};\n${await readFile('qa/privacy-provider-bundle.js','utf8')}`});let fixture=null,submitted=0,body,mismatch=false,transactionChecks=0;
 ...
 const recoveredOwner=await page.locator('#owner').textContent();await page.reload();if(spl)await page.locator('#asset').selectOption(String(index));await page.locator('#software-wallet').check();await page.locator('#unlock').click();await page.waitForFunction(()=>!document.getElementById('unlock').disabled,{},{timeout:60000});assert.equal(await page.locator('#owner').textContent(),recoveredOwner);console.log('Reload recovery: same wallet and encryption fingerprint');await page.locator('#operation').selectOption('withdraw');const recipient=Keypair.generate().publicKey.toBase58();await page.locator('#recipient').fill(recipient);await page.locator('#amount').fill(spl?'5':'0.1');if(requested){const link=createPaymentRequestLink({v:1,recipient,asset:asset.symbol,amount:spl?'5':'0.1',expires:Date.now()+3600000},'http://127.0.0.1:4173');await page.evaluate(hash=>{location.hash=hash;},new URL(link).hash);await page.locator('#request-use').click();}await page.locator('#review-payment').click();await page.waitForFunction(()=>!document.getElementById('review-payment').disabled,{},{timeout:60000});console.log('Review:',await page.locator('#privacy-status').textContent());assert.equal(await page.locator('#payment-review').isVisible(),true);assert.equal(await page.locator('#private-'+index).textContent(),spl?'10':'1');await page.locator('#acknowledge').check();await page.locator('#confirm-payment').click();await page.waitForFunction(()=>!document.getElementById('confirm-payment').disabled,{},{timeout:180000});console.log('Withdrawal:',await page.locator('#privacy-status').textContent());assert.equal(await page.locator('#tx-state').textContent(),'Finalized');assert.equal(submitted,1);assert.equal(body.recipient,recipient);assert.equal(body.mintAddress||null,asset.mint);assert.equal(requested?-body.extAmount:-body.extAmount+body.fee,spl?5000000:100000000);if(requested)console.log('Requested net amount matched the actual SDK relay payload exactly.');assert.equal(await page.evaluate(()=>window.fixtureTxSigns||0),0);
 const {raw,records}=await readHistory(page);assert.equal(records.length,1);const saved=records[0];
 assert.equal(saved.type,'withdraw');assert.equal(saved.asset,asset.symbol);assert.equal(saved.amount,requested?String(-body.extAmount+body.fee):(spl?'5000000':'100000000'));assert.equal(saved.status,'finalized');assert.equal(saved.signature,'3'.repeat(88));
