...
async function unlockWallet(page){
 ...
 assert.equal(await page.locator('#unlock-state').textContent(),'UNLOCKED');
...
try{const page=await browser.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.addInitScript({content:`window.__auditSeed=${JSON.stringify(auditSeed)};\n${await readFile('qa/privacy-provider-bundle.js','utf8')}`});let submitted=0,signature,statusChecks=0;
 ...
 assert.equal(saved.type,'deposit');assert.equal(saved.asset,spl?'USDC':'SOL');assert.equal(saved.amount,spl?'5000000':'100000000');assert.equal(saved.status,'finalized');assert.equal(saved.signature,signature);
