...
const report={checkedAt:new Date().toISOString(),mint,program:PROGRAM_ID.toBase58(),rpc:DEFAULT_RPC,sdkVersion:JSON.parse(await readFile('node_modules/privacycash/package.json','utf8')).version,sdkContainsUsdt:tokens.some(t=>t.pubkey.toBase58()===mint),sdkTokens:tokens.map(t=>({name:t.name,mint:t.pubkey.toBase58()})),usdtTree:tree(mint),usdcControlTree:tree(usdc)};
...
for(const [i,name] of ['genesis','mintAccount','pools','relayerConfig','latestSdk'].entries()){
 ...
 else if(name==='pools')report.pools={slot:v.context.slot,usdt:v.value[0]?{owner:v.value[0].owner,bytes:Buffer.from(v.value[0].data[0],'base64').length}:null,usdcControl:v.value[1]?{owner:v.value[1].owner,bytes:Buffer.from(v.value[1].data[0],'base64').length}:null};
 else if(name==='latestSdk')report.latestSdk={version:v.version,repository:v.repository};
 else report.relayerConfig=v;
}
await mkdir('qa',{recursive:true});await writeFile('qa/usdt-support-check.json',JSON.stringify(report,null,2));
